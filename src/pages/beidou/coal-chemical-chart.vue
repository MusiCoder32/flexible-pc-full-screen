...
<script>
import { noValue, debounce } from '../../utils/function-list';
...
export default {
  ...
  methods: {
    showTooltip (ref, dataIndex, seriesIndex) {
      let me = this;
      me.$refs[ref][0].chart.dispatchAction({
        type: 'showTip',
        seriesIndex,
        // name:'2020-10-20 13:00'
        dataIndex,
      });
    },
    setSensorArr () {
          ...
          'xAxis': [
            {
              ...
              'axisLabel': {
                show: Array.isArray(obj.xAxis.data) ? obj.xAxis.data.length > 0 : true,
                formatter: obj.xAxis.formatter,
              },
              'data': Array.isArray(obj.xAxis.data) && obj.xAxis.data.length > 0 ? obj.xAxis.data : null,
            },
          ],
          'yAxis': [
            {
    ...
    setOptionObj (data, type) {
      ...
      switch (type) {
        ...
        case 'SHIDU':
        case 'WENDU':
          let seriesName = (data.currValue ? data.currValue : ' - ') + data.unit + '  最新' + data.label;
          obj = {
            title: {
              show: false,
            },
            tooltip: {
              formatter: function (params) {
                console.log(params);
                let obj = params[0] || {};
                let { value, name, dataIndex, seriesIndex } = obj;
                if (value === 0) {
                  value = '无数据';
                }
                else {
